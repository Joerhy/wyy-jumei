...
transition(name="fade")
    .wrapper(v-show="show")
        mt-header.header(title="登录")
            router-link(to="/myself", slot="left")
                mt-button(icon="back")
            mt-button(slot="right") 注册
        .login-wrapper(ref="loginWrapper")
            div
                ul.login-types
                    li.login-item(v-for="(item, index) in info", :key="index")
                        span.logo
                            img(:src="item.url")
                        p.title {{item.title}}
                form.login-form
                    .other-login 或使用是手机登录
                    .input_phone
                        input(type="text", maxlength="11", placeholder="请输入正确手机号", v-model="phoneInput")
                        .register_yzm(@click="yzphone", :class="isYzActive?'yzActive':''") 验证
                    .input_yzm
                        input(type="text", maxlength="6", name="", placeholder="请输入6位验证码", v-model="yzmInput")
                    .register_jm_account 使用聚美账号登录
                    span.register_button(
                        @click="loginJM",
                        :class="isSubmit?'submitActive':''"
                    ) 登录
...
export default {
  data () {
    return {
        ...
        info: [
            {
                url:  require('../../assets/logintype/qq.png'),
                title: 'QQ'
            },{
                url:  require('../../assets/logintype/wechat.png'),
                title: '微信'
            },{
                url:  require('../../assets/logintype/alipay.png'),
                title: '支付宝'
            },{
                url:  require('../../assets/logintype/blog.png'),
                title: '新浪微博'
            },
        ],
        isYzActive: false,
