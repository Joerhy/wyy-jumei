<template lang="pug">
.page
    mt-header.header(fixed, title="未登录")
        mt-button.icon(icon="",slot="right", @click="setterClick")
            img(src="static/mine/setwhite.png")
        mt-button.icon(icon="",slot="right")
            img(src="static/mine/msgwhite.png")
    .wrapper(ref="mineWrapper")
        div
            .user-block
                .user-login
                    .user-logo
                        img(src="../../assets/bg/jumeibg2.jpg")
                    .operation(@click="loginClick", v-if="!userLogin")
                        span.signup 注册
                        span.separator
                        span.login 登录
                    .login-success(v-if="userLogin")
                        .username {{username}}
                        .autograph 我需要一个长势旺盛的个性签名
                ul.user-info
                    li.item 
                        span 0
                        p 粉丝
            ...
            .order-fund-block
                .block-title
                    span 买买买
                    span.block-title-nav 查看全部订单
                ul.order-content.content
                    li.item(v-for="(item, index) in orderContent", :key="index")
                        span.order-logo
                            img(:src="item.src")
                        p {{item.title}}
                ul.fund-content.content
                    li.item(v-for="(item, index) in fundContent", :key="index")
                        span.value {{item.number}}
                        p {{item.title}}
            .jumei-block
                ul.jumei-content
                    li.jumei-item(v-for="(item, index) in jumeiContent1", :key="index")
                        span.jumei-icon
                            img(:src="item.src")
                        p.jumei-title {{item.title}}
                ul.jumei-content
                    li.jumei-item(v-for="(item, index) in jumeiContent2", :key="index")
                        span.jumei-icon
                            img(:src="item.src")
                        p.jumei-title {{item.title}}
                ul.jumei-content
                    li.jumei-item(v-for="(item, index) in jumeiContent3", :key="index")
                        span.jumei-icon
                            img(:src="item.src")
                        p.jumei-title {{item.title}}
            .contact-block
                span.contact-img
                    img(src="../../assets/bg/valueLoad.jpg")
            //     .contact-text
            //         <span style="color: red"> Looking for internships</span> By: Jerrylee <br/> Contact:
            //         <span style="color:#FE4070">wuyuanlijie@qq.com || 15279106115</span> <br>
</template>
