<template lang="pug">
.cart-detail
    .cart-no-good(:class="hasProduct?'isHidden':''")
        .opps
            img(src="../assets/icon/pay_pop_img_loading_fail.png")
        p.empty-title 您的购物车中没有商品，请先去挑选心爱的商品吧！
        .go-shop(@click="gotoShop") 去逛逛
    .count-down(:class="hasProduct?'':'isHidden'")
        .clock-icon 
        span.time-text {{time.min}}:{{time.sec}}
        span.time-msg 结算时间，商品可能被抢空，请尽快买
    ul.groups(:class="hasProduct?'':'isHidden'")
        .g-header
            span.check-icon(@click="chooseAll=!chooseAll")
                img(:src="chooseAll?checkIcon:checkNotIcon")
            span.title 聚美优品发货
        li.group(v-for="(item, index) in cartLists", :key="index")
            span.check-icon(@click="chooseOne=!chooseOne")
                img(:src="chooseOne?checkIcon:checkNotIcon")
</template>
...
export default {
  ...
  computed: {
      cartLists() {
          ...
          added.legnth!=0?this.hasProduct=true:this.hasProduct=false;
